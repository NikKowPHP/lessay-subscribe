...
- **Initial Assessment:** If new, the app will:
  ...
  - The proficiency profile and generated lesson steps are stored in the backend.

For returning users:
- **Resuming Lessons:** The app retrieves the stored lesson steps and the user's progress so that the user can continue with the sequential lessons.
...
stepsDiagram
    ...
    alt New User
        ...
        A->>AI: Request lesson steps generation (offline)
        AI->>B: Store generated lesson steps (predefined, sequential, Natulang-style)
    else Returning User
        B->>A: Retrieve stored lesson steps and progress
    end
...
stepsDiagram
    ...
    B->>A: Update user progress and refine future lesson stepss
```
...
2. **Pre-Session Lesson Generation:**  
   - AI creates a fixed, sequential steps of lessons based on the evaluation data.
   ...
   - The complete lesson steps is stored for later delivery.
...
The high-level AI pipeline employed to generate the lesson steps offline is as follows:
...
lessons: Generated lesson stepss.
...
1. Lesson Generation:
Use the user's proficiency level, learning goals, and error patterns to generate personalized lesson stepss.
